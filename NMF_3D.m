function [C,delta,elapsed_time,res] = NMF_3D(Yr,Yt,Class_train_inx,Class_test_inx,J,MCRuns,Tol,MaxIter)

DimY = size(Yr);
Yr_mtx = reshape(Yr,[DimY(1)*DimY(2),DimY(3)]);
Yt_mtx = reshape(Yt,[DimY(1)*DimY(2),size(Yt,3)]); 
